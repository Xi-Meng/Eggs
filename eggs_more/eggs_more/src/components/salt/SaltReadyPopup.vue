...
const { activePreset, historyId } = useSaltPresetSession()
...
const popupTitle = computed(() => {
  if (activePreset.value?.label) {
    return `${activePreset.value.label}盐度合适了`
  }
...
<template>
  <div v-if="visible" class="salt-ready-popup">
    <button class="popup-backdrop" type="button" aria-label="关闭提示" @click="closePopup" />

    <section class="popup-card" role="dialog" aria-modal="true" aria-label="盐度合适提示">
      <p class="popup-kicker">烹饪提醒</p>
      <h2>{{ popupTitle }}</h2>
      <p class="popup-copy">当前盐度已经达到合适范围，可以结束烹饪了。</p>
