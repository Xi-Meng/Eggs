...
withDefaults(
  ...
  {
    title: '三分熟牛排熟了',
    copy: '当前温度已经达到设定状态，可以结束烹饪了。',
  },
)
</script>

<template>
  <div class="cook-ready-popup-preview" @click.stop.prevent @touchend.stop.prevent>
    ...
    <section class="popup-card" role="dialog" aria-modal="true" aria-label="食物熟了提示">
      ...
      <h2>{{ title }}</h2>
      <p class="popup-copy">{{ copy }}</p>
      <button class="popup-action" type="button" tabindex="-1">我知道了</button>
    </section>
  </div>
</template>

<style scoped>
.cook-ready-popup-preview {
  position: absolute;
  inset: 0;
  z-index: 12;
  pointer-events: auto;
